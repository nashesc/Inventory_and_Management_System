...
class AccountManager {
   ...
   // Initialize Super Admin (first account - you)
   ...
   login(username, password) {
      ...
      if (account) {
         ...
         // LOG THE LOGIN EVENT (only if TransactionStorage is available)
   ...
   // Approve pending account (Super Admin only)
   ...
   deleteAccount(accountId) {
      ...
      // Filter out ONLY the account being deleted (keep Super Admin and all others)
      const filteredAccounts = accounts.filter(acc => acc.id !== accountId);
      
      // Double-check Super Admin is still there
   ...
   // Get active accounts (excluding Super Admin for regular display)
   getActiveAccounts() {
...
function applyRoleRestrictions(user) {
   if (user.role === 'Staff') {
      // Hide History page from navigation
      ...
      if (document.body.classList.contains('body-settings')) {
         // Disable General Settings dropdowns
         ...
         // Hide Account Settings edit/save buttons
         ...
         // Hide Store Information edit/save buttons
      ...
      // Hide edit buttons in Inventory page
   ...
   // Hide Create Account button for non-admins
   if (user.role === 'Staff') {
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   if (logoutLink) {
      logoutLink.addEventListener('click', (e) => {
         ...
         // LOG THE LOGOUT EVENT (only if TransactionStorage is available)
         const currentUser = accountManager.getCurrentUser();
