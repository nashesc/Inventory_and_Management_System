document.addEventListener("DOMContentLoaded", () => {
   ...
   function searchProducts(query) {
      // Get fresh products from inventory each time
      ...
      searchResults.querySelectorAll('.search-result-item').forEach(item => {
         item.addEventListener('click', function() {
            ...
            saveRecentSearch(productName); // Save to recent searches
            selectProduct(
               productName,
   ...
   // Show recent searches when clicking on empty search input
   ...
   // PRINT RECEIPT → record transaction and deduct from inventory
   ...
   // Function to deduct quantity from inventory
   ...
   printBtn.addEventListener("click", () => {
      ...
      cartRows.forEach(row => {
         ...
         // Append to transaction history table
         const newRow = `
            <tr>
               <td>${formattedDate}</td>
               <td>${itemName}</td>
...
function loadSavedTransactions() {
   ...
   // Sort newest → oldest, show only sales (total > 0)
   const latest20 = all
      .filter(t => t.total > 0)
